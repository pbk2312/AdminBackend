...
public class MemberController {
    ...
    public ResponseEntity<String> deleteAccount(@RequestBody WithdrawalMembershipDTO withdrawalMembershipDTO,
                                                @CookieValue(value = "accessToken", required = false) String accessToken,
                                                HttpServletResponse response) {
        log.info("회원 탈퇴 요청...");
        Member member = memberService.getUserDetails(accessToken);
        ...
        try {
            ...
            if ("회원 정보가 정상적으로 삭제되었습니다.".equals(message)) {
                ...
                return ResponseEntity.badRequest().body(message); // 실패 메시지 반환
            }
        } catch (Exception e) {
            log.error("회원 탈퇴 중 오류 발생: {}", e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("회원 탈퇴 중 오류가 발생했습니다."); // 오류 메시지 반환
        }
    }
